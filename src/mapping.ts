...
export function handleBlocks(blocks: Block[]): Bytes {
  ...
  // this function seems not to be supported yet, so we just put it here for future reference
  // let eventsByAcctEsig_mixer: Event[] = blocks[0]
  //   .account(addr_mixer)
  //   .eventsByEsig(esig_mixer);
  let events = blocks[0].events;
  let eventsByAcctEsig_mixer: Event[] = [];
  for (let i = 0; i < events.length; i++) {
    if (events[i].address == addr_mixer && events[i].esig == esig_mixer) {
      eventsByAcctEsig_mixer.push(events[i]);
    }
  }
  if (eventsByAcctEsig_mixer.length == 0) {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    let eventsByAcctEsig_blacklist: Event[] = [];
    for (let j = 0; j < blocks[i].events.length; j++) {
      if (blocks[i].events[j].address == addr_blacklist && blocks[i].events[j].esig == esig_blacklist) {
